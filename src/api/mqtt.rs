...
impl MqttAdapter {
    ...
    pub fn start(self: Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            logger.info(&format!("MQTT client connecting to broker at {}:{}...", host, port));
            ...
            tokio::spawn(async move {
                ...
                let _ = client_sender.publish("rknx/status", QoS::AtLeastOnce, true, "online").await;
                ...
                while let Ok(event_val) = manager_rx.recv().await {
                    if let Some(addr) = event_val.get("group_address").and_then(|g| g.as_str()) {
                        ...
                        if let Err(e) = client_sender.publish(&ind_topic, QoS::AtLeastOnce, false, payload_str).await {
                            logger_sender.warn(&format!("MQTT failed to publish event indication: {}", e));
                        }
                ...
                let _ = client_sender.publish("rknx/status", QoS::AtLeastOnce, true, "offline").await;
            });
                ...
                match eventloop.poll().await {
                    Ok(notification) => {
                        if let Event::Incoming(Packet::Publish(publish)) = notification {
                            ...
                            tokio::spawn(async move {
                                let _ = self_inner.handle_command(&manager_inner, &client_inner, &topic, &payload).await;
                            });
                        }
                    }
                    Err(e) => {
    ...
    async fn handle_command(&self, manager: &ApiManager, client: &AsyncClient, topic: &str, payload: &str) -> Result<(), &'static str> {
        ...
        let result: Result<Value, String> = match cmd {
            "connect" => {
                let conn_type = parsed_json.get("connection_type").and_then(|c| c.as_str()).unwrap_or("");
            ...
            "subscribe" => {
                let addr = if parsed_json.is_string() {
                    ...
                    parsed_json.get("group_address").and_then(|g| g.as_str()).unwrap_or("")
                };
            ...
            "set_dpt" => {
                let addr = parsed_json.get("group_address").and_then(|g| g.as_str()).unwrap_or("");
                let dpt = parsed_json.get("dpt").and_then(|d| d.as_str()).unwrap_or("");
            ...
            "set_retention" => {
                let seconds = if parsed_json.is_number() {
                    ...
                    parsed_json.get("seconds").and_then(|s| s.as_i64()).unwrap_or(604800)
                };
        ...
        let _ = client.publish(&response_topic, QoS::AtLeastOnce, false, response_payload.to_string()).await;
        Ok(())
    }
}
