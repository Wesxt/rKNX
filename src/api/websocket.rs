...
impl WebSocketServer {
    ...
    pub fn start(self: Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            let listener = match TcpListener::bind(&addr).await {
                Ok(l) => l,
                Err(e) => {
                    logger.error(&format!("Failed to bind WebSocket server to {}: {}", addr, e));
                    return;
                }
            };
            ...
            while let Ok((stream, client_addr)) = listener.accept().await {
                ...
                tokio::spawn(async move {
                    if let Ok(ws_stream) = accept_async(stream).await {
                        logger_cloned.info(&format!("New WebSocket client connected from {}", client_addr));
                        let (mut ws_sender, mut ws_receiver) = ws_stream.split();
                        let mut manager_events_rx = manager_cloned.subscribe_events();

                        // Loop for sending events to this client
                        let (event_sender_tx, mut event_sender_rx) = tokio::sync::mpsc::channel::<Message>(100);
                        ...
                        tokio::spawn(async move {
                            while let Some(msg) = event_sender_rx.recv().await {
                                if let Err(e) = ws_sender.send(msg).await {
                                    logger_sender.warn(&format!("Failed to send WebSocket message: {}", e));
                                    break;
                                }
                            }
                        });
                        ...
                        while let Some(Ok(msg)) = ws_receiver.next().await {
                            if let Message::Text(text) = msg {
                                let response = self_cloned_inner.handle_client_message(&manager_cloned, &text).await;
                                let _ = event_sender_tx.send(Message::Text(response.to_string())).await;
                            }
                        }

                        logger_cloned.info(&format!("WebSocket client from {} disconnected", client_addr));
                    }
                });
            }
        });
    ...
    async fn handle_client_message(&self, manager: &ApiManager, text: &str) -> Value {
        ...
        let result = match action {
            "connect" => {
                let conn_type = req.get("connection_type").and_then(|c| c.as_str()).unwrap_or("");
            ...
            "subscribe" => {
                let addr = req.get("group_address").and_then(|g| g.as_str()).unwrap_or("");
                if addr.is_empty() {
            ...
            "set_retention" => {
                let seconds = req.get("seconds").and_then(|s| s.as_i64()).unwrap_or(604800);
